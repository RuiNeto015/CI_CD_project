...
export class PostService extends BaseAPI implements IPostService {
    ...
    public async getPostBySlug(slug: string): Promise<APIResponse<Post>> {
        try {
            ...
            console.log('inside getbyslug', response)
    ...
    public addTags(tags: TagModel[], slug: string) {

    }

    public async createPost(title: string, type: PostType, text?: string, link?: string, tags?: TagModel[]): Promise<APIResponse<void>> {
        try {
            const result = await this.post('/posts', {title, postType: type, text, link}, null, {
                authorization: this.authService.getToken('access-token')
            })
            console.log('result post', result.data.postSlug)
            console.log('tags', tags)
            if (tags) {
                const tagsName = tags.map(t => t.tag);
                await this.post('/tags/post', {postSlug: result.data.postSlug, tags: tagsName}, null, {
                    authorization: this.authService.getToken('access-token')
                })
            }
